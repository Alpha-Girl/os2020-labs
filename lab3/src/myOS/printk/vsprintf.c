...
static char *number(char *str, int num, int base, int size, int precision, int type)
{
	...
	int i;
	// 若类型 type 指出用小写字母，则定义小写字母集。若类型指出要左调整(靠左边界)，则屏蔽填零标志。
	// 若进制基数小于 2 或大于 36，则退出处理，也即本程序只能处理基数在 2-32 之间的数
	if (type & SMALL)
	...
	if (base < 2 || base > 36)
		return 0;
	// 若类型指出要填零，则置字符变量 c='0' (即 ''),否则 c 等于空格字符。若类型指出是带符号数并且
	// 数值 num 小于 0，则置符号变量 sign=负号，并使 num 取绝对值。否则如果类型指出是加号，
	// 则置 sign=加号，否则若类型带空格标志则 sign=空格，否则置 0
	c = (type & ZEROPAD) ? '0' : ' ';
	if (type & SIGN && num < 0)
	{
		sign = '-';
	...
	if (i > precision)
		precision = i;
	size -= precision;
	// 从这里真正开始形成所需要的转换结果，并暂时放在字符串 str 中。若类型中没有填零(ZEROPAD)
	// 和左调整标志，则在 str 中首先填放剩余宽度值指出的空格数。若需带符号位，则存入符号。
	...
	if (!(type & LEFT))
		while (size-- > 0)
			*str++ = c;
	// 此时 i 存有数值 num 的数字个数。若数字个数小于精读值，则 str 中放入(精度值 - i)个 '0'.
	while (i < precision--)
		*str++ = '0';
	while (i-- > 0) // 将转换好的数字字符填入 str 中。共 i 个。
		*str++ = tmp[i];
...
int vsprintf(char *buf, const char *fmt, va_list args)
{
	...
	repeat:
		...
		// 标志域 '-' 标志(左靠齐)，因此需在标志变量中添入该标志，并将字段宽度值取为其绝对值。
		...
		// 下面代码取格式转换串精度域，并放入 precision 变量中。精度域的开始标志是  '.'。其处理过程与上面
		// 宽度域类似。若精度域中是数值则直接取其为精度值。若精度域中是字符 '*'，表示下一个参数指定精度。
		// 因此调用 va_arg 取精度值。若此时宽度值小于 0，则将字段精度值取为 0.
		// get the precision
		precision = -1;
		if (*fmt == '.')
		{
		...
		case 'c':
			...
			*str++ = (uint8_t)va_arg(args, int);
			while (--field_width > 0)
				*str++ = ' ';
			break;
		// 如果转换指示符是 's',则表示对应参数是字符串。首先取参数字符串的长度，若其超过了精度域值，则
		...
		// 位置处还有字符，则也直接将该字符写入输出串中，并返回到 117 行处继续处理格式字符串。否则表示
